...
const Footer: React.FC = () => {
  return (
    <footer className="mb-10 ">
      <div className="flex lg:justify-around flex-col md:flex-row lg:items-center md:justify-center md:items-center md:gap-x-20 lg:gap-x-10 gap-y-10 flex-wrap xl:flex-nowrap container mx-auto ">
        <ul>
          <li className="flex gap-x-2 items-center">
            ...
            <span className="text-lg text-black font-bold font-Abril">
              Dribes
            </span>
          </li>
          <li className="text-sm md:text-lg  w-52 md:w-64">
            We are a rent a car agency that will help you fins best car you have
            been dream to ride, lets
          </li>
        ...
        <ul>
          {FooterData1.map(({ id, name }) => {
            return (
              <li
                key={id}
                className={`${id == 1 ? 'text-xl font-bold mb-4  text-secondary' : 'text-lg text-success'} text-sm md:text-lg`}
              >
                {name}
              </li>
            );
      ...
      <p className="text-center text-success mt-10">
        ©2024 All Rights Reserved by Dribes
      </p>
    </footer>
  );
};
